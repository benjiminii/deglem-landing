...
<a href="/" class="flex items-center gap-2 transition-opacity hover:opacity-80">
  <img src="/logo/deglem-glyph-dark.png" alt="" width="40" height="40" class="h-10 w-10" />
  <span class="text-2xl font-bold text-charcoal">Deglem</span>
</a>
```
The glyph `<img>` uses empty `alt=""` (decorative) because the adjacent "Deglem" text already names the brand. Header lockup is `h-10` glyph + `text-2xl` wordmark; the footer uses a smaller `h-5` glyph.

### Store badges
Charcoal pills (`180×54`, `rx=10`) carrying the brand glyph + two-line label, displayed at `h-[54px] w-auto`:
- App Store: Apple glyph + "Download on the / App Store"
- Google Play: Play-triangle glyph + "GET IT ON / Google Play"
Rendered via `StoreBadges.astro`, which links each to the config URL. These are **stylized in-house badges** — swap the official Apple / Google artwork before public launch for brand-guideline compliance.
...
- **Store badges:** stylized charcoal SVGs with the Apple / Play glyph + two-line label (see §6 Store badges). Swap the official Apple / Google artwork before launch (brand-guideline compliance).
